...
public class UIEnd2EndTest : PageTest
{
    ...
    public async Task End2EndTest2()
    {
        ...
        await Page.GetByPlaceholder("username").ClickAsync();
        //CHANGE NUMBER HERE
        await Page.GetByPlaceholder("username").FillAsync("Alan11");
        await Page.GetByPlaceholder("name@example.com").ClearAsync();
        //CHANGE NUMBER HERE
        await Page.GetByPlaceholder("name@example.com").FillAsync("Alan11@mail.dk");
        await Page.GetByLabel("Password", new() { Exact = true }).ClickAsync();
        await Page.GetByLabel("Password", new() { Exact = true }).FillAsync("Halløj1!");
        await Page.GetByLabel("Confirm Password").ClickAsync();
        await Page.GetByLabel("Confirm Password").FillAsync("Halløj1!");
        ...
        //Checkign That Alan is logged in and can log out
        await Expect(Page.GetByRole(AriaRole.Link, new() { Name = "Logout Alan11" })).ToBeVisibleAsync();
        //Checking that the noot box is visible
        //CHANGE NUMBER HERE
        await Expect(Page.GetByText("Noot noot? Alan11? Share")).ToBeVisibleAsync();
        ...
        //CHANGE NUMBER HERE
        await Page.GetByRole(AriaRole.Link, new() { Name = "Logout Alan11" }).ClickAsync();
        await Page.GetByRole(AriaRole.Button, new() { Name = "Click here to Logout" }).ClickAsync();
